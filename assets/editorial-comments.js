...
editorialCommentReply = {
	...
	show: function (xml) {
		...
		if (response.action.indexOf('reply') == -1 || !ef_thread_comments) {
			...
			jQuery('#ef-comments').prepend(comment);
		} else {

			// This is a reply, so add it after the comment replied to
			parent_id = jQuery("#ef-comment_parent").val();
			if (jQuery(`#comment-${parent_id}`).next().is("ul")) {
				// Already been replied to, so just add to the list
				jQuery(`#comment-${parent_id}`).next().prepend(comment);
			} else {
				...
				newUL.insertAfter(`#comment-${parent_id}`)
			}
		}
